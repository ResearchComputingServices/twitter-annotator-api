...
def add_tweet(data_dic):
    ...
    media_keys) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) """
    record_to_insert = (data_dic['tweet_id'], data_dic['original_tweet_id'], data_dic['text'], data_dic['date'], 
                        ...
                        data_dic['verified_author'], data_dic['media_keys'])
...
def add_data(file_path):
    ...
    global tw
    global row,header
    '''header = ['tweet_id', 'tweet_text (may be clipped if too long and retweeted)',
              'full original tweet text if available','tweet date','tweet time', 'source used to tweet', 
              'tweet language', 'author username', 'author name', 'autor profile image url', 
              'author description', 'if author is verified']'''
                ...
                for retweeted_tweet in all_responses['includes']['tweets']:
                    if retweeted_tweet['id'] == retweet_id:
                        if retweeted_tweet['lang'] == 'en':
                            data_dic['lang'] = 'en'
                            data_dic['tweet_id'] = retweet_id
                            data_dic['text'] = retweeted_tweet['text']
                            
    
                            created_at = retweeted_tweet['created_at']
                            date_list = created_at.split('T')
                            date,tim = date_list[0],date_list[1].split('.')[0]
                            data_dic['time'] = tim
                            data_dic['date'] = date
                            data_dic['source'] = retweeted_tweet['source']
                            
                            author_id = retweeted_tweet['author_id']
                            ...
                            if 'attachments' in retweeted_tweet.keys():
                                if 'media_keys' in retweeted_tweet['attachments'].keys():
                                    media_keys = ''
                                    for media_key in retweeted_tweet['attachments']['media_keys']:
